...
{                                                                        \
	gen_helper_hsim_ld##SUFF (val, cpu_env, addr);						\
}

#define DO_GEN_ST(SUFF, OPC)                                             \
static inline void gen_aa32_st##SUFF(TCGv_i32 val, TCGv_i32 addr, int index) \
{                                                                        \
	gen_helper_hsim_st##SUFF(cpu_env, val, addr);							\
}

#else // not CONFIG_HSIM
...
{
	gen_helper_hsim_ld64(val, cpu_env, addr);
}

static inline void gen_aa32_st64(TCGv_i64 val, TCGv_i32 addr, int index)
{
	gen_helper_hsim_st64(cpu_env, val, addr);
}
